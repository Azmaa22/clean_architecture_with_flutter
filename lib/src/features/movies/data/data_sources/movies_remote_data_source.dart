...
import 'package:clean_architecture_with_flutter/src/features/movies/data/models/movie_details_model.dart';
import 'package:clean_architecture_with_flutter/src/features/movies/data/models/movie_model.dart';
import 'package:clean_architecture_with_flutter/src/features/movies/data/models/movie_recommendations_model.dart';
...
class MoviesRemoteDataSource extends BaseMoviesRemoteDataSource {
  ...
  Future<MovieDetailsModel> getMovieDetails(int parameters) async {
    String path =
        '${EndPoints.getMovieDetails}/$parameters?api_key=${AppConstances.apiKey}';
    final response = await Dio().get(path);
    if (response.statusCode == 200) {
  ...
  Future<List<MovieRecommendationModel>> getMovieRecommendations(
    ...
    String path =
        '${EndPoints.getMovieRecommendations}/$parameters/recommendations?api_key=${AppConstances.apiKey}';
    final response = await Dio().get(path);
    if (response.statusCode == 200) {
      return List<MovieRecommendationModel>.from(
        (response.data['results'] as List).map(
          (movieData) => MovieRecommendationModel.fromJson(movieData),
        ),
      );
    } else {
      throw ServerException(
